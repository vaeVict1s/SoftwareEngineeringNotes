...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \hline
        \multirow{1}{*}{\textbf{How to}} & a request is turned into a 
                                           \highlightit{command object} 
                                           that stores the receiver and 
                                           the operation parameters; \\
                                         & an \highlightit{invoker} 
                                           triggers \texttt{execute()} 
                                           without knowing how the work 
                                           is done, so commands can be 
...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \hline
        \multirow{1}{*}{\textbf{How to}} & traversal logic is moved into a 
                                           separate \highlightit{iterator} 
                                           object that exposes 
                                           \texttt{next}/\texttt{hasNext} 
                                           (or equivalent); \\
                                         & the collection only creates 
                                           iterators, so clients walk 
                                           elements without knowing the 
...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \multirow{1}{*}{\textbf{What for}} & \highlightit{reducing chaotic and 
                                             ...
                                             the objects \\
        \hline
        \multirow{1}{*}{\textbf{How to}} & components talk only to a 
                                           \highlightit{mediator} 
                                           interface instead of to each 
                                           other; \\
                                         & the concrete mediator 
                                           encapsulates interaction 
                                           rules and routes calls, so 
                                           adding a component does not 
                                           wire a new mesh of 
                                           dependencies\\
        \hline
...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \multirow{1}{*}{\textbf{What for}} & \highlightit{saving/restoring 
                                             ...
                                             internals\\
        \hline
        \multirow{1}{*}{\textbf{How to}} & the \highlightit{originator} 
                                           creates an opaque 
                                           \highlightit{memento} that 
                                           stores its internal state; \\
                                         & a \highlightit{caretaker} 
                                           keeps mementos and asks the 
                                           originator to restore from 
                                           one, without reading the 
                                           saved fields itself\\
        \hline
...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \hline
        \multirow{1}{*}{\textbf{How to}} & a \highlightit{publisher} 
                                           keeps a list of 
                                           \highlightit{subscribers} 
                                           and notifies them on state 
                                           changes; \\
                                         & subscribers implement a 
                                           common update interface and 
                                           register/unregister at 
                                           runtime, so the publisher 
                                           never hard-codes their types\\
        \hline
...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \multirow{1}{*}{\textbf{What for}} & implementing 
                                             ...
                                             internal state}\\
        \hline
        \multirow{1}{*}{\textbf{How to}} & state-specific behavior is 
                                           extracted into separate 
                                           \highlightit{state} classes 
                                           that share an interface; \\
                                         & the context holds a current 
                                           state object and delegates 
                                           calls to it, switching the 
                                           reference when the state 
                                           changes\\
        \hline
...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \hline
        \multirow{1}{*}{\textbf{How to}} & each algorithm variant becomes 
                                           a \highlightit{strategy} 
                                           class behind a shared 
                                           interface; \\
                                         & the context holds a strategy 
                                           reference and delegates the 
                                           varying step to it, so the 
...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \hline
        \multirow{1}{*}{\textbf{How to}} & a base class defines a 
                                           \highlightit{template method} 
                                           that calls a fixed sequence 
                                           of steps; \\
                                         & invariant steps stay in the 
                                           base class, while 
                                           \highlightit{primitive/hook} 
                                           steps are abstract or 
                                           virtual so subclasses fill 
...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \multirow{1}{*}{\textbf{What for}} & \highlightit{separating algorithms 
                                             ...
                                             operate on\\
        \hline
        \multirow{1}{*}{\textbf{How to}} & each element class accepts a 
                                           \highlightit{visitor} and 
                                           calls back 
                                           \texttt{visit(this)}; \\
                                         & new operations are added as 
                                           new visitor classes, while 
                                           the element hierarchy stays 
                                           closed to that change 
                                           (double dispatch)\\
        \hline
...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \hline
        \multirow{1}{*}{\textbf{How to}} & an \highlightit{adapter} 
                                           implements the interface 
                                           clients expect and 
                                           \highlightit{translates} 
                                           calls to the adaptee's API; \\
                                         & clients depend only on the 
                                           target interface, so the 
                                           foreign class can be reused 
...
\begin{table}[h!t]
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \hline
        \multirow{1}{*}{\textbf{How to}} & the \highlightit{abstraction} 
                                           hierarchy holds a reference 
                                           to an 
                                           \highlightit{implementation} 
                                           interface and delegates 
                                           low-level work to it; \\
                                         & both hierarchies can evolve 
