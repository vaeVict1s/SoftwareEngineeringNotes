...
\subsection{Amortized vs average-case}
\label{subsec:amortized-vs-average}
These two phrases sound alike and are \textbf{not} the same.

\begin{itemize}
    \item \textbf{Average-case / expected} (e.g.\ \texttt{unordered\char`_map::find}):
          the bound is over a \highlight{distribution of inputs} (or a random
          hash). With a good hash, most lookups cost constant time; a bad
          collision pattern can make one lookup $\Theta(n)$. The
          \emph{expected} cost per op is $O(1)$; the \emph{worst} single op
          is $O(n)$. See Chapter~\ref{chap:containers}.
    \item \textbf{Amortized} (e.g.\ \texttt{vector::push\char`_back}):
          the bound is over a \highlight{sequence of operations} on one
          structure. One \texttt{push\char`_back} may reallocate and cost
          $\Theta(n)$; most cost $\Theta(1)$. With geometric growth, $n$
          pushes cost $O(n)$ \emph{total}, so the \highlight{amortized}
          cost per push is $O(1)$. A single push can still be $\Theta(n)$
          worst-case.
\end{itemize}

\par\noindent\textbf{Interview tip:} say which kind you mean.
``\texttt{find} is expected $O(1)$, worst $O(n)$'' is about hashing;
``\texttt{push\char`_back} is amortized $O(1)$'' is about spreading rare
reallocations across many cheap inserts. Monotonic stack / Union--Find
``amortized $O(1)$ per index'' uses the same idea: each element is charged
a constant number of expensive pops over the whole run.
...
\newline
\highlight{Feed level $\neq$ local storage:} you can subscribe to L2 or L3 and still keep only 
L1 (or a shallow depth window) in memory --- store what the strategy reads on the hot path.
\newline
...
\toclessnparagraph{Pointer chasing vs flat layout}
\label{par:order-book-flat-vs-pointer}
Node-based structures (\texttt{std::map}, linked lists, pointer-heavy trees) store each 
level or order in a separately allocated node. Walking the book then 
\highlight{pointer-chases}: load a node, follow \texttt{left}/\texttt{next}, miss cache, 
repeat. Under a burst of depth updates or a strategy that scans several levels, those 
misses dominate --- not the $O(\log n)$ compare count on the whiteboard.
\newline
\textbf{Noise removed} by flat, preallocated contiguous storage: random node hops on the 
hot path. A \highlight{tick-indexed ladder} (or dense \texttt{vector} of levels over a 
bounded price window) keeps neighbouring prices in neighbouring cache lines so hardware 
prefetch helps. Pair with pools for L3 order nodes if you still need per-id queues 
(Section~\ref{sec:avoiding-allocations}; Chapter~\ref{chap:cacheperformance}).
\newline
\textbf{Cost / when a map is fine:} a sparse book over a huge price range wastes RAM on a 
dense ladder; whiteboard sketches and moderate update rates often keep 
\texttt{std::map}. Choose the layout for the instrument's tick grid and the strategy's 
read pattern (BBO only vs deep walk), not as a universal rule.
\begin{lstlisting}[language=C++]
// Sketch: bids as qty[tick], tick = price / tick_size (bounded window).
constexpr int kTicks = 4096;
alignas(64) int64_t bid_qty[kTicks]{};  // preallocated; index = px_to_tick(px)
// BBO: track best_tick explicitly on each update --- do not scan 4096 slots.
\end{lstlisting}
\par\noindent\textbf{Interview tip:} ``pointer chasing vs contiguous ladder'' --- name the miss, 
then the bound on price range that makes the array legal.

\toclessnparagraph{Aggregate book (L2-style storage)}
Quantity (and optional order count) per price --- no per-order queues. Matches an L2 feed, 
or an L3 feed that you collapse locally.
\begin{lstlisting}[language=C++]
#include <cstdint>
#include <map>
...
\toclessnparagraph{Order-by-order book (L3-style storage)}
...
struct PriceLevel {
    int64_t total_qty{0};
    std::deque<Order> queue;  // price-time FIFO at this price
};
...
picking the structure. In real HFT stacks, generic \texttt{std::map} is often replaced by 
tick-indexed arrays or pooled trees when the price range is bounded 
(Chapter~\ref{chap:quantbasics}).

\subsection{Add, Update, and Remove}
\label{subsec:order-book-operations}
The feed does not hand you a finished book --- it streams \highlight{deltas}. Your hot path 
is applying those deltas correctly and cheaply. Wrong apply logic is silent corruption 
(ghost levels, wrong BBO); expensive apply logic is jitter under burst.
\newline
...
\end{itemize}
\textbf{Invariants to keep every tick:} no empty levels left in the maps; BBO cache 
invalidated or updated when the top level changes; order-id index consistent if you track 
individual orders. A missed empty-level cleanup is a classic ``phantom liquidity'' bug.
\newline
\textbf{Noise removed} by treating modify as cancel+add when the venue semantics allow it: 
one code path, fewer special cases. \textbf{Cost:} you must still match the venue's exact 
rules (in-place qty change vs new priority).
\newline
Sequence numbers / gaps: market-data feeds are often UDP; detect gaps and rebuild from 
snapshot (Chapter~\ref{chap:linuxnetworking}). Applying increments after a gap without a 
resync is how books drift from the venue.
\par\noindent\textbf{Interview tip:} walk one add that creates a new best bid, then a cancel 
that restores the old BBO --- and say which caches you touch.
...
The book update loop sits on the critical path between packet and decision. Asymptotic 
``$\log n$ map'' talk is not enough if the real noise is allocator locks, false sharing, 
or a mutex inside the decode loop.
\newline
\textbf{Noise removed} by the practices below: allocation jitter, cache-line ping-pong on 
shared book state, and scheduler wake-ups from contended locks.
\begin{itemize}
    ...
    \item \highlightbf{Contiguous levels} and stable layout beat pointer-chasing lists when 
          depth walks matter (see pointer chasing vs flat layout above; 
...
\end{itemize}
\textbf{Cost:} more upfront sizing and ownership discipline; you give up casual 
multi-threaded mutation of one book. That trade is usually correct for tick-to-trade.
\par\noindent\textbf{Interview tip:} name the noise first (alloc, false share, lock), then the 
knob --- not a laundry list of buzzwords.
