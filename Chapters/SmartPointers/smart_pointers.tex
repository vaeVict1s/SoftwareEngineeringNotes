...
Only \highlightbf{non-const \texttt{std::unique\char`_ptr}} can 
\highlightit{transfer the ownership} of the managed object to \highlightbf{another 
\texttt{std::unique\char`_ptr}}.
\newline
If an object's lifetime is managed by a 
\highlightbf{const \texttt{std::unique\char`_ptr}}, it is limited to 
the scope in which the pointer was created.

\highlightbf{\texttt{std::unique\char`_ptr}} is commonly used to manage the lifetime 
