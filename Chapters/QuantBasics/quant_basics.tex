...
\begin{enumerate}
    \item \textbf{Sections~\ref{sec:quant-dev-role}--\ref{sec:instruments}:} 
          Role, lifecycle, market structure, and the instrument map 
          (spot through swaps).
...
\newline
\noindent\textit{Roadmap (markets expansion):} spot/futures infrastructure (C), 
complex derivatives systems notes (D), crypto (E), and microstructure polish (F) 
remain; deep order-book layout stays in Chapter~\ref{chap:algorithms}. 
Done so far: market structure (A,~Section~\ref{sec:market-structure}) and 
instruments (B,~Section~\ref{sec:instruments}).
...
\begin{itemize}
    ...
    \item \highlightbf{CME} (Chicago Mercantile Exchange Group) --- futures and options 
          ...
          products and rolls matter for data (Phase~4 / Section~C).
    \item \highlightbf{Crypto exchanges} (e.g.\ Binance, Coinbase, Deribit for options) 
          ...
          differ from traditional CME/NYSE stacks (Phase~5).
\end{itemize}
...
Section~\ref{sec:plain-options}; infrastructure differences (spot vs futures wires, 
rolls, implieds) come in Phase~4 / Section~C.
...
\begin{itemize}
    ...
    \item \highlightbf{Crypto} --- digital assets; spot, futures, perpetuals, options; 
          24/7 venues (compact treatment in Phase~5).
\end{itemize}
...
(Phase~4). FX spot shops (e.g.\ prop FX) often stay in continuous OTC/ECN spot because 
that \emph{is} the product they trade --- futures are an alternative listing, not a 
universal upgrade.
